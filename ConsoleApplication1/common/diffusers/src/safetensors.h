...
safetensors_file_init(void* file_buffer, int64_t file_buffer_bytes, safetensors_File* out) {
	*out = safetensors_File(); // 使用构造函数初始化
	...
	int header_len = 0;
	{
		...
		mem_copy(&header_len_u64, file_buffer, sizeof(header_len_u64));
		if (header_len_u64 > (uint64_t)INT_MAX)
			return (char*)"File header allegedly more than INT_MAX bytes, file likely corrupt";
		header_len = header_len_u64;
	}
	assert(header_len >= 0);
	if (header_len == 0)
	...
	char* t = ((char*)file_buffer) + 8; // 跳过文件头的前8个字节
	char* e = t + header_len; // 计算 header 的结束位置
	out->one_byte_past_end_of_header = e; // 保存 header 结束位置

	char* tensor_data_baseptr = t + header_len; // 计算张量数据的基地址
